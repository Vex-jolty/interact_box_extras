...
AutorunToggleWidget::AutorunToggleWidget(wxWindow* parent, wxWindowID id) {
	...
	Bind(wxEVT_CHECKBOX, &AutorunToggleWidget::OnChange, this, id);
}
...
bool AutorunToggleWidget::getAutorunStatus() {
	...
	#if WINVER > _WIN32_WINNT_NT4
	result =
		RegQueryValueEx(hKey, _interactBoxKeyName.c_str(), 0, NULL, (LPBYTE)szBuffer, &dwBufferSize);
	#else
	result =
		RegQueryValueExA(hKey, _interactBoxKeyName.c_str(), 0, NULL, (LPBYTE)szBuffer, &dwBufferSize);
	#endif
...
bool AutorunToggleWidget::toggleAutorun(bool autorunIsEnabled) {
	try {
		...
		long result = RegOpenKeyEx(
			HKEY_CURRENT_USER, _regKeyToOpen.c_str(), 0, KEY_SET_VALUE | KEY_QUERY_VALUE, &hKey
		);
	#else
		...
		result = RegSetValueEx(
			hKey, _interactBoxKeyName.c_str(), 0, REG_SZ, (BYTE*)val.c_str(),
