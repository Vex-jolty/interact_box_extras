...
int MyFrame::getSettingPositionPriorityByType(const Json::Value& setting) {
	if (setting.isString()) return 1;
	if (setting.isInt64() || setting.isUInt64()) return 2;
	if (setting.isArray()) return 3;
	if (setting.isBool()) return 4;
	return 5;
}

int MyFrame::getSettingPositionPriorityByName(const string& key, const std::unordered_map<std::string, int>& namePriorities) {
	auto it = namePriorities.find(key);
	if (it != namePriorities.end()) return it->second;
	return namePriorities.size() + 1;
}


vector<pair<string, Json::Value>> MyFrame::sortSettings(Json::Value& initialSettings) {
	unordered_map<std::string, int> namePriorities = {
		{"host", 1},
		{"port", 2}
	};
	...
	std::sort(entries.begin(), entries.end(),
		[&namePriorities, this](const auto& a, const auto& b) {
...
int InteractBoxSettings::OnExit() {
	return 0;
}

MyFrame::MyFrame() : wxFrame(NULL, wxID_ANY, "Interact Box Settings", wxDefaultPosition, wxSize(400, 600)) {
	...
	auto scrollPanel = new wxScrolledWindow(parentPanel, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxVSCROLL);
...
void MyFrame::parseSettings(wxPanel* panel, wxBoxSizer* sizer) {
	...
	for (auto [name, val] : _sortedSettings) {
		...
		// Yes, this is terrible. It sucks to have so many else if statements, and nested if statements, but it's the best I could do.
		if (value.isBool()) {
			...
			if (name.ends_with("Dir")) {
				sizer->Add(new wxStaticText(panel, id * 100, StringHelper::camelCaseToHuman(name + "ectory", true)));
