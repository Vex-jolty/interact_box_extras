...
class TimeThread : public wxThread {
	public:
		TimeThread(MainAppFrame* handler) : wxThread(wxTHREAD_DETACHED) { handlerPointer = handler; }
		~TimeThread();
...
class ProcessKillerThread : public wxThread {
	public:
		ProcessKillerThread(
			MainAppFrame* handler,
			std::vector<ProcessNameAndFileName> processNamesAndFiles
		)
				: wxThread(wxTHREAD_DETACHED), handlerPointer(handler),
