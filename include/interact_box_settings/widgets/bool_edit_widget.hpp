...
class BoolEditWidget : public wxCheckBox {
	public:
		BoolEditWidget(wxWindow* parent, wxWindowID id, std::string name, Json::Value& jsonSettings)
				: _name(name), _jsonSettings(jsonSettings),
					_label(StringHelper::camelCaseToHuman(name, true)) {
			Create(parent, id, _label);
			SetValue(_jsonSettings[_name].asBool());
			Bind(wxEVT_CHECKBOX, &BoolEditWidget::onChange, this, id);
		}
